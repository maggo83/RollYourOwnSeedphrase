/*
 * Minimal SHA-256 implementation for Uint8Array inputs.
 * Implements FIPS PUB 180-4 directly and has no runtime dependencies.
 */
(function (root) {
  'use strict';
  ...
  function sha256(input) {
    ...
    const bitLengthHigh = Math.floor(inputLength / 0x20000000);
    const bitLengthLow = (inputLength << 3) >>> 0;
    const lengthView = new DataView(padded.buffer);
    lengthView.setUint32(paddedLength - 8, bitLengthHigh, false);
    ...
    for (let offset = 0; offset < paddedLength; offset += 64) {
      ...
      for (let index = 0; index < 64; index += 1) {
        ...
        const temp1 = (h + sum1 + choice + K[index] + schedule[index]) >>> 0;
  ...
  root.OfflineHash = Object.freeze({ sha256 });
